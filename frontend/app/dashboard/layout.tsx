...
import { DashboardUserProvider, getUserDisplayName } from "./user-context";
...
export default function DashboardLayout({
  ...
  return (
    <DashboardUserProvider user={user} isLoadingUser={!user && !error}>
        ...
        <header className={"my-4"}>
          <div className="mx-auto flex w-full max-w-full h-9 px-4 items-center justify-between gap-4">
            ...
            <div className="hidden flex-1 text-sm text-stone-600 md:block">
              {error ? <span>{error}</span> : <span>{user ? `Signed in as ${displayName}` : "Loading profile..."}</span>}
            </div>
